...
(1) While running some descriptives on the dataset, I found that there was `r count(distinct(nyc_transit,line, station_name))` distinct stations (defined by both the station name and the line). (2) Additionally, I found that there was 468 observations which are ADA compliant. (3) Lastly, among the entrance/exits that don't allow vending, 37.7% of them allow entry.
...
Once we merged these three dataframes, the final dataset contains `r nrow (prob3_df)` and `r ncol(prob3_df)`. This final dataframe contains all the same columns as in the 3 merged dataframes. The year range in this dataframe is